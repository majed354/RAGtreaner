"""Runtime guard helpers for application-side legal-mode outputs.

This layer is used inside the real MLX application path, not just benchmarks.
For now we only enable it on legal memos because benchmark validation showed a
clear gain there, while applying the same logic to legal analysis hurt answer
quality.
"""
