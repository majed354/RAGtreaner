"""Build a tiny v15 anti-loop polish dataset on top of v13.

The goal is narrower than v14:

- preserve the strong opinion gains from v13
- reduce late-output looping / repetition in memo and analysis
- keep the dataset very small and conservative
"""
...
def find_section_positions(text: str, sections: list[str]) -> dict[str, int]:
...
def section_body_lengths(text: str, sections: list[str]) -> dict[str, int]:
    positions = find_section_positions(text, sections)
    ordered = sorted(((name, idx) for name, idx in positions.items()), key=lambda item: item[1])
    lengths: dict[str, int] = {}
    for idx, (name, start) in enumerate(ordered):
        body_start = start + len(name)
        body_end = ordered[idx + 1][1] if idx + 1 < len(ordered) else len(text)
        lengths[name] = max(0, len(text[body_start:body_end].strip()))
    return lengths


def late_sections_have_substance(text: str, sections: list[str], minimum_body: int) -> bool:
    lengths = section_body_lengths(text, sections)
    return all(lengths.get(section, 0) >= minimum_body for section in sections)


def is_memo_polish_candidate(example: dict[str, Any]) -> bool:
    ...
    if section_count(text, LATE_MEMO_HEADINGS) < len(LATE_MEMO_HEADINGS):
        return False
    if not late_sections_have_substance(text, LATE_MEMO_HEADINGS, 70):
        return False
    if unique_article_refs(text) < 2:
        return False
    return 1200 <= len(text) <= 2800


def is_analysis_polish_candidate(example: dict[str, Any]) -> bool:
    ...
    if section_count(text, LATE_ANALYSIS_HEADINGS) < len(LATE_ANALYSIS_HEADINGS):
        return False
    if not late_sections_have_substance(text, LATE_ANALYSIS_HEADINGS, 60):
        return False
    if unique_article_refs(text) < 2:
        return False
    return 900 <= len(text) <= 2400
...
def is_opinion_anchor(example: dict[str, Any]) -> bool:
    if detect_mode(example) != "legal_opinion" or not base_usable(example):
        return False
    text = assistant_text(example)
    if section_count(text, ["النظام المنطبق", "الحكم المباشر", "المواد المستند إليها", "القيود أو الاستثناءات", "ما لم يثبته النص", "الخلاصة العملية"]) < 6:
        return False
    if unique_article_refs(text) < 2:
        return False
    return 650 <= len(text) <= 1300


def sort_memo_candidate(example: dict[str, Any]) -> tuple[int, int, int, int, str]:
    text = assistant_text(example)
    late_lengths = section_body_lengths(text, LATE_MEMO_HEADINGS)
    return (
        section_count(text, MEMO_HEADINGS),
        section_count(text, LATE_MEMO_HEADINGS),
        min(late_lengths.values()) if late_lengths else 0,
...
def sort_analysis_candidate(example: dict[str, Any]) -> tuple[int, int, int, int, str]:
    text = assistant_text(example)
    late_lengths = section_body_lengths(text, LATE_ANALYSIS_HEADINGS)
    return (
...
def build_manifest(
    *,
    ...
    payload = {
            "dataset_version": "v15_loop_polish",
        "sources": sources,
        "strategy": strategy,
...
def main() -> None:
    ...
    parser.add_argument("--memo-repeat", type=int, default=1)
    parser.add_argument("--analysis-limit", type=int, default=12)
    parser.add_argument("--analysis-repeat", type=int, default=1)
    parser.add_argument("--opinion-anchor-limit", type=int, default=16)
    parser.add_argument("--valid-per-mode", type=int, default=3)
    parser.add_argument("--test-per-mode", type=int, default=3)
    ...
    build_manifest(
        ...
        strategy={
            "recipe": "v13_plus_memo_analysis_anti_loop_polish_with_opinion_anchors",
